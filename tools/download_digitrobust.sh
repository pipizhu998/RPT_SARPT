...
FILE_ID="${DIGITROBUST_GOOGLE_DRIVE_FILE_ID:-16fNf_HxBpfPLioLrN2v6wM5YYTTUN9a4}"
ARCHIVE_NAME="${DIGITROBUST_ARCHIVE_NAME:-digitrobust.tar.gz}"
DATASET_DIR="${DIGITROBUST_DATASET_DIR:-datasets/DigitRobust}"

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd -- "${SCRIPT_DIR}/.." && pwd)"
ARCHIVE_PATH="${PROJECT_ROOT}/${ARCHIVE_NAME}"
TMP_DIR="${PROJECT_ROOT}/.tmp/download_digitrobust"
COOKIE_JAR="${TMP_DIR}/gdrive_cookies.txt"
CONFIRM_PAGE="${TMP_DIR}/gdrive_confirm.html"
MANIFEST="${TMP_DIR}/digitrobust_manifest.txt"

FORCE_DOWNLOAD=0
if [[ "${1:-}" == "--force" ]]; then
  FORCE_DOWNLOAD=1
elif [[ "${1:-}" != "" ]]; then
  echo "Usage: tools/download_digitrobust.sh [--force]" >&2
  exit 2
fi
...
download_dataset_archive() {
  if [[ -z "${FILE_ID}" ]]; then
    echo "DIGITROBUST_GOOGLE_DRIVE_FILE_ID is empty." >&2
...
extract_dataset_archive
echo "Done. DigitRobust is under ${PROJECT_ROOT}/${DATASET_DIR}"
